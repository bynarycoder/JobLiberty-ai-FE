...
import { Rocket, Clock, ExternalLink, Bookmark } from "lucide-react";
...
export function FellowshipsSection({ fellowships, isLoading, bookmarkedIds, onToggleBookmark }: FellowshipsSectionProps) {
  const { t } = useI18n();

  if (isLoading) return <SkeletonCards count={3} columns={3} />;
  if (fellowships.length === 0) return <EmptyState titleKey="opportunityHub.emptyState.title" descriptionKey="opportunityHub.emptyState.description" />;

  return (
    <section aria-label={t("opportunityHub.fellowships.title")}>
      <h2 className="mb-5 text-xl font-semibold tracking-tight">{t("opportunityHub.fellowships.title")}</h2>
            ...
            <Card className="h-full transition-shadow hover:shadow-md">
              <CardContent className="p-5">
                <div className="flex items-start gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-violet-100 text-sm font-semibold text-violet-700 dark:bg-violet-900/30 dark:text-violet-300">
